...
pub fn process_resource_queue(
    ...
    for request in requests {
        ...
        let data_value: LuaValue = match lua_ctx.lua.registry_value(&request.data) {
            Ok(value) => value,
            Err(e) => {
                error!("Failed to retrieve Lua value for resource {}: {}", request.resource_name, e);
                continue;
            }
        };
        ...
        else if let Some(result) = serde_registry.try_insert_resource(&request.resource_name, &data_value, world) {
            if let Err(e) = result {
                error!("Failed to insert resource {}: {}", request.resource_name, e);
            }
        } else {
            warn!("Resource type '{}' is not registered in builder or serde registry", request.resource_name);
        }
        
        // Remove the registry value to free memory
        if let Err(e) = lua_ctx.lua.remove_registry_value(request.data) {
            warn!("Failed to remove registry value for resource {}: {}", request.resource_name, e);
        }
    }
}
