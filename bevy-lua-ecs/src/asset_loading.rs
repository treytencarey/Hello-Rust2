use bevy::asset::{ReflectAsset, RenderAssetUsages, UntypedAssetId};
use bevy::prelude::*;
...
pub type AssetCloner =
    Box<dyn Fn(&mut World, &dyn std::any::Any) -> Option<UntypedHandle> + Send + Sync>;
...
/// ```
pub fn parse_enum_from_string<
    E: bevy::reflect::GetTypeRegistration + bevy::reflect::FromReflect + Default + Clone,
>(
    variant_name: &str,
) -> Option<E> {
...
pub fn register_cloner_if_clone<T: Asset + Clone + 'static>(
    ...
    let cloner: AssetCloner = Box::new(
        |world: &mut World, asset: &dyn std::any::Any| -> Option<UntypedHandle> {
...
pub fn register_asset_types_runtime(
    ...
    trace!(
        "[RUNTIME_ASSET] Attempting to register {} discovered asset type names",
    ...
    for type_name in discovered_names {
        ...
        let Some(reflect_asset) = type_registration.data::<ReflectAsset>() else {
            trace!(
                "[RUNTIME_ASSET] {} found but is not an Asset type (no ReflectAsset data)",
                type_name
            );
            continue;
        };
        ...
        // Uses ReflectAsset::add to add assets via reflection
        {
            ...
            let adder: AssetAdder = Box::new(
                move |world: &mut World, asset: &dyn Reflect| -> Option<UntypedHandle> {
                    // ReflectAsset::add takes &dyn Reflect and adds to Assets<T>
                    let handle = reflect_asset_for_typeid.add(world, asset);
                    debug!(
                        "[RUNTIME_ASSET] Added asset of type {} -> handle {:?}",
                        type_path_clone_1, handle
                    );
                    Some(handle)
                },
            );
            ...
            asset_registry
                ...
                .lock()
                .unwrap()
                .insert(type_path.clone(), adder_string);
        }
        ...
        {
            let creator: HandleCreator =
                Box::new(move |untyped: UntypedHandle| -> Box<dyn PartialReflect> {
                    // Return the UntypedHandle boxed - it implements Reflect
                    Box::new(untyped) as Box<dyn PartialReflect>
                });

            asset_registry
                .handle_creators_by_typeid
                .lock()
                .unwrap()
                .insert(handle_type_id, creator);
        }

        // 6. Mark as registered
        registered.insert(type_name.to_string());
        debug!(
            "[RUNTIME_ASSET] ✓ Registered asset type: {} (TypeId: {:?})",
            type_name, asset_type_id
        );
    }
    ...
    debug!(
        "[RUNTIME_ASSET] Registered {} asset types at runtime: {:?}",
        registered.len(),
        registered_names
    );
}
...
impl AssetRegistry {
    ...
    pub fn from_type_registry(type_registry: &AppTypeRegistry) -> Self {
        ...
        debug!(
            "✓ Registered {} handle setters for asset types",
            handle_setters.len()
        );
        ...
        for registration in registry.iter() {
            if registration.data::<ReflectAsset>().is_some() {
                ...
                let creator: HandleCreator = Box::new(move |untyped: UntypedHandle| {
                    ...
                    debug!(
                        "[HANDLE_DISCOVER] Creating handle for asset type: {}",
                        creator_type_path
                    );
                ...
                discovered_count += 1;
                debug!(
                    "[ASSET_DISCOVER] ✓ Discovered asset type: {} -> {}",
                    type_path, handle_type_path
                );
                ...
                if !handle_setters.contains_key(&handle_type_path) {
                    debug!("[ASSET_DISCOVER] ⚠ Handle setter not registered for {} (requires compile-time registration)", handle_type_path);
                }
            }
        }
    ...
    pub fn discover_entity_components(type_registry: &AppTypeRegistry) -> Vec<(String, String)> {
        ...
        for registration in registry.iter() {
            ...
            let is_entity_wrapper = match type_info {
                TypeInfo::TupleStruct(ts) if ts.field_len() == 1 => ts
                    .field_at(0)
                    .map(|f| f.type_path().contains("Entity"))
                    .unwrap_or(false),
                TypeInfo::Struct(s) if s.field_len() == 1 => s
                    .field_at(0)
                    .map(|f| f.type_path().contains("Entity"))
                    .unwrap_or(false),
                _ => false,
            ...
            if is_entity_wrapper {
                debug!(
                    "[ENTITY_DISCOVER] ✓ Found Entity wrapper component: {} ({})",
        ...
        if !entity_components.is_empty() {
            debug!(
                "[ENTITY_DISCOVER] Auto-discovered {} Entity wrapper components",
    ...
    pub fn try_create_typed_handle_box(
        ...
        debug!(
            "[HANDLE_CREATE] Looking for creator for type: '{}'",
            inner_type_path
        );

        if let Some(creator) = handle_creators.get(inner_type_path) {
            debug!("[HANDLE_CREATE] ✓ Found creator for '{}'", inner_type_path);
            Some(creator(untyped_handle))
        } else {
            debug!(
                "[HANDLE_CREATE] ✗ No creator found for '{}'. Registered paths:",
                inner_type_path
            );
    ...
    pub fn try_wrap_in_newtype(
        ...
        debug!(
            "[NEWTYPE_WRAP] Looking for wrapper for type: '{}'",
            newtype_type_path
        );

        if let Some(wrapper) = wrappers.get(newtype_type_path) {
            debug!("[NEWTYPE_WRAP] ✓ Found wrapper for '{}'", newtype_type_path);
            Some(wrapper(typed_handle))
        } else {
            debug!(
                "[NEWTYPE_WRAP] ✗ No wrapper found for '{}'. Registered paths:",
                newtype_type_path
            );
    ...
    pub fn try_wrap_in_newtype_with_reflection(
        ...
        debug!(
            "[NEWTYPE_WRAP_REFLECT] Looking for wrapper for type: '{}'",
            newtype_type_path
        );
        ...
        // Fast path: check registered wrappers first
        {
            ...
            if let Some(wrapper) = wrappers.get(newtype_type_path) {
                debug!(
                    "[NEWTYPE_WRAP_REFLECT] ✓ Found registered wrapper for '{}'",
                    newtype_type_path
                );
        ...
        let registration = registry.get_with_type_path(newtype_type_path).or_else(|| {
            ...
            let short_name = newtype_type_path
                .rsplit("::")
                .next()
                .unwrap_or(newtype_type_path);
            registry.get_with_short_type_path(short_name)
        });

        let Some(registration) = registration else {
            debug!(
                "[NEWTYPE_WRAP_REFLECT] ✗ Type '{}' not found in TypeRegistry",
                newtype_type_path
            );
            return None;
        };
        ...
        let Some(reflect_from_reflect) = registration.data::<ReflectFromReflect>() else {
            debug!(
                "[NEWTYPE_WRAP_REFLECT] ✗ Type '{}' has no ReflectFromReflect registered",
                newtype_type_path
            );
            return None;
        };
        ...
        let dynamic_value: Box<dyn PartialReflect> = match type_info {
            ...
            bevy::reflect::TypeInfo::Struct(struct_info) => {
                ...
                for field in struct_info.iter() {
                    ...
                    if !handle_inserted && field_type_path.contains("Handle<") {
                        ...
                        if input_type_path.contains("UntypedHandle") {
                            ...
                            if let Some(creator) = handle_creators.get(field_type_path) {
                                ...
                                if let Some(untyped_handle) =
                                    typed_handle.try_downcast_ref::<UntypedHandle>()
                                {
                                ...
                                if let Ok(cloned) = typed_handle.reflect_clone() {
                                    dynamic_struct
                                        .insert_boxed(field_name, cloned.into_partial_reflect());
                                    handle_inserted = true;
                            ...
                            if let Ok(cloned) = typed_handle.reflect_clone() {
                                ...
                                handle_inserted = true;
                                debug!(
                                    "[NEWTYPE_WRAP_REFLECT] Inserted handle into field '{}'",
                                    field_name
                                );
                            } else {
                        ...
                        if let Some(ref default_inst) = default_instance {
                            ...
                            let default_field =
                                match default_inst.as_partial_reflect().reflect_ref() {
                            ...
                            if let Some(field_type_registration) = registry.get(field.type_id()) {
                                if let Some(reflect_default) =
                                    field_type_registration.data::<bevy::prelude::ReflectDefault>()
                                {
                                    let default_value = reflect_default.default();
                                    dynamic_struct.insert_boxed(
                                        field_name,
                                        default_value.into_partial_reflect(),
                                    );
                                    debug!("[NEWTYPE_WRAP_REFLECT] Used ReflectDefault for field '{}': {}", 
                                        field_name, field_type_path);
                                } else if let Some(from_reflect) =
                                    field_type_registration.data::<ReflectFromReflect>()
                                {
                                    ...
                                    let attempts: &[Box<dyn PartialReflect>] = &[
                                        ...
                                        Box::new({
                                            let mut t = DynamicTupleStruct::default();
                                            t.insert(1.0f32);
                                            t
                                        }),
                                        Box::new({
                                    ...
                                    for attempt in attempts {
                                        if let Some(default_value) =
                                            from_reflect.from_reflect(attempt.as_ref())
                                        {
                                            dynamic_struct.insert_boxed(
                                                field_name,
                                                default_value.into_partial_reflect(),
                                    ...
                                    if !created {
                                        ...
                                        debug!("[NEWTYPE_WRAP_REFLECT] ⚠ Skipping field '{}', hoping from_reflect fills default: {}", 
                                            field_name, field_type_path);
                                    }
                                } else {
                                    debug!("[NEWTYPE_WRAP_REFLECT] ⚠ No default mechanism for field '{}': {}", 
                                        field_name, field_type_path);
                                }
                            }
                        }
                ...
                if !handle_inserted {
                    debug!(
                        "[NEWTYPE_WRAP_REFLECT] ✗ No Handle<T> field found in struct '{}'",
                        newtype_type_path
                    );
                    return None;
                }
                ...
                debug!(
                    "[NEWTYPE_WRAP_REFLECT] Built DynamicStruct for '{}' with {} fields",
                    newtype_type_path,
            ...
            _ => {
                debug!("[NEWTYPE_WRAP_REFLECT] ✗ Type '{}' is neither TupleStruct nor Struct, unsupported: {:?}", 
        ...
        match reflect_from_reflect.from_reflect(dynamic_value.as_ref()) {
            Some(concrete) => {
                debug!("[NEWTYPE_WRAP_REFLECT] ✓ Auto-discovered newtype wrapper for '{}' via reflection", newtype_type_path);

                // Cache the successful discovery
                self.discovered_newtype_cache
                    .lock()
                    .unwrap()
                    .insert(newtype_type_path.to_string());

                Some(concrete.into_partial_reflect())
            }
            None => {
                debug!(
                    "[NEWTYPE_WRAP_REFLECT] ✗ ReflectFromReflect::from_reflect failed for '{}'",
                    newtype_type_path
                );
                None
            }
    ...
    pub fn try_add_asset(
        ...
        if has_adder {
            debug!("[ASSET_ADD] ✓ Found adder for '{}'", type_name);
            ...
            let adders = self.asset_adders.lock().unwrap();
            debug!(
                "[ASSET_ADD] ✗ No adder found for '{}'. Registered adders: {:?}",
                type_name,
                adders.keys().collect::<Vec<_>>()
            );
        }
        None
    }
    ...
    pub fn try_clone_and_add_asset(
        ...
        let has_cloner = self
            .asset_cloners_by_typeid
            .lock()
            .unwrap()
            .contains_key(&type_id);

        if has_cloner {
            debug!("[ASSET_CLONE] ✓ Found cloner for TypeId {:?}", type_id);
            ...
            trace!(
                "[ASSET_CLONE] ✗ No cloner registered for TypeId {:?}",
                type_id
            );
        }
        None
    ...
    pub fn get_asset_constructor(
        &self,
        type_name: &str,
    ) -> Option<Box<dyn Fn(&mlua::Table) -> LuaResult<Box<dyn Reflect>> + Send + Sync>> {
...
pub fn process_pending_assets(world: &mut World) {
    ...
    let bitflags_registry = world
        .get_resource::<crate::bitflags_registry::BitflagsRegistry>()
        .cloned();
        ...
        if let Some(registration) = registry.get_with_type_path(&pending_asset.type_name) {
            ...
            if let Some(reflect_asset) = registration.data::<ReflectAsset>() {
                ...
                let asset_result = if pending_asset.type_name == "bevy_mesh::mesh::Mesh" {
                    ...
                    match create_mesh_from_primitive(
                        &pending_asset.data,
                        &lua_ctx.lua,
                        &type_registry,
                    ) {
                        Ok(mesh) => {
                            if let Some(mesh_ref) = mesh.as_reflect().downcast_ref::<Mesh>() {
                                debug!(
                                    "[ASSET_PROCESS] Mesh created successfully, vertices: {}",
                                    mesh_ref.count_vertices()
                                );
                            } else {
                                debug!(
                                    "[ASSET_PROCESS] Mesh created, but downcast to Mesh failed!"
                                );
                            }
                            Some(mesh)
                        }
                    ...
                    let constructor_result: Option<LuaResult<Box<dyn Reflect>>> = {
                        if let Ok(lua_value) =
                            lua_ctx.lua.registry_value::<LuaValue>(&pending_asset.data)
                        {
                    ...
                    if let Some(result) = constructor_result {
                        debug!(
                            "[ASSET_PROCESS] Using registered constructor for {}",
                        ...
                        match result {
                            ...
                            Err(e) => {
                                error!(
                                    "Asset constructor failed for {}: {}",
                                    pending_asset.type_name, e
                                );
                                None
                            }
                        }
                        ...
                        debug!(
                            "[ASSET_PROCESS] Created default for {}, now populating from Lua",
                            pending_asset.type_name
                        );
                ...
                if let Some(mut asset) = asset_result {
                            ...
                            if pending_asset.type_name == "bevy_image::image::Image" {
                                if let Some(image) = asset.as_any_mut().downcast_mut::<Image>() {
                                    if let Ok(usages_str) = t.get::<String>("texture_usages") {
                                        let flags: Vec<&str> =
                                            usages_str.split('|').map(|s| s.trim()).collect();
                                        ...
                                        for flag in flags {
                                            match flag {
                                                ...
                                                _ => {
                                                    warn!("[IMAGE] Unknown texture usage: {}", flag)
                                                }
                                            }
                                        }
                                        image.texture_descriptor.usage =
                                            TextureUsages::from_bits_truncate(usage_bits);
                                        debug!(
                                            "[IMAGE] ✓ Set texture_usages to {:?}",
                                            image.texture_descriptor.usage
                                        );
                                    }
                                }
                    ...
                    let untyped_handle = if let Some(direct_add) =
                        registration.data::<ReflectDirectAssetAdd>()
                    {
                        if let Some(handle) = direct_add.add(asset.as_any(), world) {
                            debug!("[ASSET_PROCESS] ✓ Used ReflectDirectAssetAdd for {} (preserves ALL data)", pending_asset.type_name);
                            handle
                        } else {
                            debug!("[ASSET_PROCESS] ReflectDirectAssetAdd downcast failed for {}, trying fallbacks", pending_asset.type_name);
                            // Continue to next fallback
                            if let Some(handle) = asset_registry.try_clone_and_add_asset(
                                world,
                                asset_type_id,
                                asset.as_any(),
                            ) {
                                debug!(
                                    "[ASSET_PROCESS] ✓ Used registered cloner for {}",
                                    pending_asset.type_name
                                );
                                handle
                            } else if let Some(handle) = asset_registry.try_add_asset(
                                world,
                                &pending_asset.type_name,
                                asset.as_ref(),
                            ) {
                                debug!(
                                    "[ASSET_PROCESS] Used registered adder for {}",
                                    pending_asset.type_name
                                );
                                handle
                            } else {
                        ...
                        asset_registry.try_clone_and_add_asset(world, asset_type_id, asset.as_any())
                    {
                        debug!("[ASSET_PROCESS] ✓ Used registered cloner for {} (preserves non-Reflect data)", pending_asset.type_name);
                        handle
                    } else if let Some(handle) = asset_registry.try_add_asset(
                        world,
                        &pending_asset.type_name,
                    ...
                    debug!(
                        "[ASSET_PROCESS] Asset {} added, handle ID: {:?}",
                        pending_asset.type_name,
                        untyped_handle.id()
                    );
                    ...
                    asset_registry.register_asset_handle(
                        id,
                        pending_asset.type_name.clone(),
                        untyped_handle.id(),
                    );

                    debug!(
                        "✓ Created asset {} with ID {} -> Handle {:?}",
                        pending_asset.type_name,
                        id,
                        untyped_handle.id()
                    );
                ...
                error!(
                    "Type {} is not an asset (missing ReflectAsset)",
                    pending_asset.type_name
                );
            }
        } else {
            ...
            error!(
                "Available asset types in registry: {:?}",
                registry
                    .iter()
...
fn create_mesh_from_primitive(
    ...
    let data_table = match lua_value {
        ...
        _ => {
            return Err(LuaError::RuntimeError(
                "Mesh data must be a table".to_string(),
            ))
        }
    };
    ...
    let primitive_table = match primitive_value {
        ...
        _ => {
            return Err(LuaError::RuntimeError(
                "Mesh must have 'primitive' field".to_string(),
            ))
        }
    };

    // Get the shape variant (e.g., "Cuboid", "Sphere", "Circle")
    let pairs: Vec<(String, LuaValue)> = primitive_table.pairs().collect::<LuaResult<_>>()?;

    if pairs.is_empty() {
        return Err(LuaError::RuntimeError(
            "Primitive must specify a shape".to_string(),
        ));
    }
    ...
    debug!(
        "[MESH_GEN] Looking up primitive type: {}",
        primitive_type_path
    );
    ...
    let registration = registry
        .get_with_type_path(&primitive_type_path)
        .ok_or_else(|| {
            LuaError::RuntimeError(format!(
                "Primitive type not found: {}. Available primitives in registry: {:?}",
                primitive_type_path,
                registry
                    .iter()
    ...
    let mut primitive_instance =
        if let Some(reflect_default) = registration.data::<ReflectDefault>() {
            ...
            reflect_from_reflect
                ...
                .ok_or_else(|| {
                    LuaError::RuntimeError(format!(
                        "Failed to create primitive {} from reflection",
                        shape_name
                    ))
                })?
        } else {
            return Err(LuaError::RuntimeError(format!(
                "Primitive type {} doesn't support Default or FromReflect",
                shape_name
            )));
        };
    ...
    let mesh: Mesh = match shape_name.as_str() {
        "Cuboid" => {
            ...
            if let Some(cuboid) = primitive_instance.downcast_ref::<Cuboid>() {
                ...
                return Err(LuaError::RuntimeError(
                    "Failed to downcast to Cuboid".to_string(),
                ));
            }
        }
        "Sphere" => {
            ...
            if let Some(sphere) = primitive_instance.downcast_ref::<Sphere>() {
                ...
                return Err(LuaError::RuntimeError(
                    "Failed to downcast to Sphere".to_string(),
                ));
            }
        }
        "Cylinder" => {
            ...
            if let Some(cylinder) = primitive_instance.downcast_ref::<Cylinder>() {
                ...
                return Err(LuaError::RuntimeError(
                    "Failed to downcast to Cylinder".to_string(),
                ));
            }
        }
        "Capsule3d" => {
            ...
            if let Some(capsule) = primitive_instance.downcast_ref::<Capsule3d>() {
                ...
                return Err(LuaError::RuntimeError(
                    "Failed to downcast to Capsule3d".to_string(),
                ));
            }
        }
        "Plane3d" => {
            ...
            if let Some(plane) = primitive_instance.downcast_ref::<Plane3d>() {
                ...
                return Err(LuaError::RuntimeError(
                    "Failed to downcast to Plane3d".to_string(),
                ));
            }
        }
        "Torus" => {
            ...
            if let Some(torus) = primitive_instance.downcast_ref::<Torus>() {
                ...
                return Err(LuaError::RuntimeError(
                    "Failed to downcast to Torus".to_string(),
                ));
            }
        }
        _ => {
    ...
    debug!(
        "[MESH_GEN] Created {} mesh with {} vertices",
        shape_name,
        mesh.count_vertices()
    );
    debug!("[MESH_GEN] Topology: {:?}", mesh.primitive_topology());
    debug!(
        "[MESH_GEN] Has POSITION: {}",
...
fn construct_asset_from_lua(
    ...
    let data_table = match lua_value {
        ...
        _ => {
            return Err(LuaError::RuntimeError(
                "Asset data must be a table".to_string(),
            ))
        }
    };
    ...
    let asset_registry = lua
        .globals()
        .get::<LuaTable>("__asset_registry")
        .ok()
    ...
    let reflect_from_reflect = registration.data::<ReflectFromReflect>().ok_or_else(|| {
        LuaError::RuntimeError(format!(
            "Asset type {} doesn't implement FromReflect",
            registration.type_info().type_path()
        ))
    })?;

    let asset = reflect_from_reflect
        .from_reflect(&dynamic_struct)
        .ok_or_else(|| {
            LuaError::RuntimeError(format!(
                "Failed to create asset {} from reflection",
                registration.type_info().type_path()
            ))
        })?;
...
fn try_create_value_from_lua(
    ...
    use bevy::reflect::{DynamicEnum, DynamicStruct, DynamicVariant};
                ...
                if let LuaValue::Table(variant_data) = value {
                    ...
                    for pair in variant_data.pairs::<String, LuaValue>() {
                        ...
                        if let Some(nested_value) =
                            try_create_value_from_lua(field_value, asset_registry)?
                        {
                    ...
                    let dynamic_enum =
                        DynamicEnum::new(key, DynamicVariant::Struct(dynamic_struct));
...
fn populate_asset_from_lua(
    ...
    debug!(
        "[ASSET_POPULATE] ReflectMut variant: {:?}",
    ...
    if let bevy::reflect::ReflectMut::Struct(struct_mut) = reflect_result {
        debug!(
            "[ASSET_POPULATE] Struct has {} fields",
            struct_mut.field_len()
        );
        ...
        for i in 0..struct_mut.field_len() {
            ...
            if let Some(field_name) = field_name_opt {
                ...
                if let Ok(lua_val) = data_table.get::<LuaValue>(&*field_name) {
                    debug!(
                        "[ASSET_POPULATE]   Found Lua value for '{}': {:?}",
                        field_name, lua_val
                    );
                    if let Some(field) = struct_mut.field_at_mut(i) {
                        ...
                        if let LuaValue::Integer(asset_id) = &lua_val {
                            ...
                            if type_path.contains("Handle<") {
                                ...
                                if let Some(handle) =
                                    asset_registry.get_untyped_handle(*asset_id as u32)
                                {
                                    if asset_registry.try_set_handle_field(
                                        ...
                                        debug!("[ASSET_POPULATE] ✓ Set {} via registered handle setter", field_name);
                                        continue;
                                    } else {
    ...
    match lua_value {
            ...
            if let Ok(string) = s.to_str() {
                ...
                if string.contains('|') {
                    ...
                    if try_set_bitflags_from_names(field, &flag_names, bitflags_registry) {
                        debug!("[SET_BASIC_FIELD] ✓ Applied bitflags: {}", string);
                        return Ok(());
                    }
                }
        ...
        LuaValue::Table(table) => {
            ...
            match field.reflect_mut() {
                ...
                _ => {
                    debug!(
                        "Lua table for non-struct field type: {:?}",
...
fn lua_value_to_reflect_value(
    ...
    match lua_value {
        ...
        _ => Err(LuaError::RuntimeError(format!(
            "Cannot convert {:?} to reflect value",
            lua_value
        ))),
    }
}
                        ...
                        bevy::reflect::VariantInfo::Tuple(tuple_variant) => {
                            ...
                            if tuple_variant.field_len() == 1 {
                                ...
                                let final_value = if is_newtype_wrapper {
                                    ...
                                    let mut dynamic_ts =
                                        bevy::reflect::DynamicTupleStruct::default();
                        ...
                        bevy::reflect::VariantInfo::Struct(struct_variant) => {
                            ...
                            if let LuaValue::Table(variant_table) = variant_value {
                                ...
                                for field_info in struct_variant.iter() {
                                    if let Ok(lua_val) =
                                        variant_table.get::<LuaValue>(field_info.name())
                                    {
                                        if let Ok(reflect_val) =
                                            lua_value_to_reflect_value(&lua_val)
                                        {
                                            variant_struct
                                                .insert_boxed(field_info.name(), reflect_val);
                                        }
                                    }
                                }
...
pub fn add_asset_loading_to_lua(
    ...
    let load_asset = lua.create_function(
        move |lua_ctx, (path, options): (String, Option<LuaTable>)| {
            ...
            if file_exists {
                ...
                if network_enabled && should_reload {
                    ...
                    if has_loader {
                        debug!(
                            "📥 [LOAD_ASSET] Queuing background server check for: {}",
                            path
                        );
                ...
                if has_loader {
                    debug!(
                        "📥 [LOAD_ASSET] File not found locally, requesting download: {}",
                        path
                    );
                    script_cache_clone.register_pending_download_coroutine(
                        path.clone(),
                    ...
                    Err(LuaError::RuntimeError(format!(
                        "Asset not found: {} (network downloads not enabled)",
                        path
                    )))
                }
            } else {
    ...
    let load_asset_async = lua.create_function(
        move |lua_ctx, (path, callback, options): (String, LuaFunction, Option<LuaTable>)| {
            use mlua::Value as LuaValue;
            ...
            let current_parent_id: u64 =
                lua_ctx.globals().get::<u64>("__INSTANCE_ID__").unwrap_or(0);
            ...
            if file_exists {
                ...
                if network_enabled && should_reload {
                    ...
                    if has_loader {
                        debug!(
                            "📥 [load_asset_async] Queuing background server check for: {}",
                            path
                        );
                ...
                if has_loader {
                    debug!(
                        "📥 [load_asset_async] File not found locally, queuing download: {}",
                        path
                    );
    ...
    let create_asset =
        lua.create_function(move |lua_ctx, (type_name, data): (String, LuaTable)| {
...
fn apply_post_construction_bitflags(
    ...
    for (key, value) in pairs {
        ...
        if apply_bitflags_to_nested_field(asset, &key, &flag_names, type_name, bitflags_registry) {
            debug!(
                "[POST_CONSTRUCTION] ✓ Applied {} to {}: {}",
                key, type_name, value_str
            );
        } else {
            debug!(
                "[POST_CONSTRUCTION] ⚠ Failed to apply {} to {}: {}",
                key, type_name, value_str
            );
        }
    }
...
fn apply_bitflags_to_nested_field(
    ...
    if let ReflectMut::Struct(s) = asset.reflect_mut() {
        for i in 0..s.field_len() {
            ...
            if field_name.contains("descriptor") || field_name.contains(target_field_short) {
                if let Some(field_ref) = s.field_at_mut(i) {
                    ...
                    if try_apply_bitflags_value(
                        field_ref.as_partial_reflect_mut(),
                        target_field_short,
                        flag_names,
                        bitflags_registry,
                    ) {
        ...
        if let Some(field) = s.field_mut(target_field_short) {
            if try_set_bitflags_from_names(
                field.as_partial_reflect_mut(),
                flag_names,
                bitflags_registry,
            ) {
                return true;
                ...
                if let Some(reflect_mut) = field_ref.as_partial_reflect_mut().try_as_reflect_mut() {
                    if let ReflectMut::Struct(nested) = reflect_mut.reflect_mut() {
                        if let Some(usage_field) = nested.field_mut(target_field_short) {
                            if try_set_bitflags_from_names(
                                usage_field.as_partial_reflect_mut(),
                                flag_names,
                                bitflags_registry,
                            ) {
                                return true;
    ...
    if let Some(reflect_mut) = field.try_as_reflect_mut() {
        if let ReflectMut::Struct(s) = reflect_mut.reflect_mut() {
            ...
            if let Some(nested_field) = s.field_mut(target_name) {
                if try_set_bitflags_from_names(
                    nested_field.as_partial_reflect_mut(),
                    flag_names,
                    bitflags_registry,
                ) {
                    return true;
...
fn try_set_bitflags_from_names(
    ...
    if type_path.contains("Flags") || type_path.contains("Usage") {
        debug!(
            "[POST_CONSTRUCTION] Unknown bitflags type: {} - register via BitflagsRegistry",
            type_path
        );
    }
