...
pub fn process_spawn_queue(
    ...
    for request in requests {
        ...
        debug!("[SPAWN_QUEUE] Registered temp_id {} -> {:?}", request.temp_id, entity_id);
            ...
            // Check if it's a known Rust component (Reflect)
                ...
                let resolved_data = if let LuaValue::Table(ref table) = data_value {
                    if let Ok(entity_id) = table.get::<u64>("entity") {
                        ...
                        debug!("[SPAWN_QUEUE] Resolved entity reference: {} -> {:?} (bits: {})", 
                            entity_id, resolved_entity, resolved_entity.to_bits());
        ...
        if let Some(parent_id) = request.parent_temp_id {
            ...
            debug!("[SPAWN_QUEUE] Setting parent for {:?}: {} -> {:?}", entity_id, parent_id, parent_entity);
