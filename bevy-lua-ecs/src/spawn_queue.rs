...
impl SpawnQueue {
    ...
    pub fn generate_temp_id(&self) -> u64 {
        self.next_temp_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed)
    }
    ...
    pub fn register_entity(&self, temp_id: u64, entity: Entity) {
        self.temp_id_to_entity.lock().unwrap().insert(temp_id, entity);
    }
    ...
    pub fn get_entity(&self, temp_id: u64) -> Option<Entity> {
        self.temp_id_to_entity.lock().unwrap().get(&temp_id).copied()
    }
    ...
    pub fn set_parent(&self, temp_id: u64, parent_temp_id: u64) {
        ...
        for request in queue.iter_mut() {
            if request.temp_id == temp_id {
                ...
                debug!("[SPAWN_QUEUE] Set parent {} for temp_id {}", parent_temp_id, temp_id);
                return;
            }
        }
    }
    ...
    pub fn add_pending_component(&self, temp_id: u64, component_name: String, data: LuaRegistryKey) {
        let mut queue = self.queue.lock().unwrap();
        for request in queue.iter_mut() {
            if request.temp_id == temp_id {
                ...
                debug!("[SPAWN_QUEUE] Added pending component {} for temp_id {}", component_name, temp_id);
                return;
            }
        }
        // If the spawn request isn't found, log a warning (might have already been processed)
        warn!("[SPAWN_QUEUE] Could not find spawn request for temp_id {} to add component {}", temp_id, component_name);
    }
    ...
    pub fn register_observer_callback(&self, temp_id: u64, event_type: String, callback: LuaRegistryKey) {
        let mut callbacks = self.observer_callbacks.lock().unwrap();
        callbacks.entry(temp_id).or_default().push((event_type, callback));
    }
    ...
    pub fn take_observer_callbacks(&self, temp_id: u64) -> Vec<(String, LuaRegistryKey)> {
        self.observer_callbacks.lock().unwrap()
            .remove(&temp_id)
            .unwrap_or_default()
    }
