...
function Prediction.system(world)
    ...
    local entities = world:query({
        with = { NetSync.MARKER, NetSync.PREDICTION, "Transform", "PlayerState", "ScriptOwned" }
    })
    for _, entity in ipairs(entities) do
        local sync = entity:get(NetSync.MARKER)
        local pred = entity:get(NetSync.PREDICTION)
        ...
        if last_acked > (pred.last_acked_sequence or 0) then
            ...
            entity:patch({ [NetSync.PREDICTION] = pred })
        end
