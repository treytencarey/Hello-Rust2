...
function Server.send_system(world)
    ...
    for _, msg_entity in ipairs(outbound) do
        ...
        -- print(string.format("[NET_SERVER] Sending message (%s): %s", channel == Messages.CHANNEL_RELIABLE and "RELIABLE" or "UNRELIABLE", encoded))
...
function Server.on_client_connected(client_id, world)
    print(string.format("[NET_SERVER] Client %s connected", client_id))
    ...
    print(string.format("[NET_SERVER] Sent %d entities to client %s", count, client_id))
end
