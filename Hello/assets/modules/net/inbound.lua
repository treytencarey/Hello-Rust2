...
local function handle_spawn(world, msg, owner_client)
    ...
    if State.is_known(net_id) then
        print(string.format("[NET] Spawn skipped - entity %d already known", net_id))
    ...
    if existing_id then
        print(string.format("[NET] Adopting existing entity %d for net_id %d", existing_id, net_id))
    ...
    if parent_net_id then
        ...
        if not parent_entity_id then
            ...
            print(string.format("[NET] Deferred spawn of %d waiting for parent %d", net_id, parent_net_id))
            return
        end
    end
    ...
    if owner_client == State.my_client_id() then
        ...
        print("[NET] Added PredictionState for own entity")
    end
    ...
    print(string.format("[NET] Spawned entity %d with net_id %d", entity_id, net_id))
...
function Inbound.handle_update(world, msg, owner_client)
    ...
    if not entity_id then
        print(string.format("[NET] Update for unknown entity %d", net_id))
        return
    end
    
    local entity = world:get_entity(entity_id)
    if not entity then
        print(string.format("[NET] Entity %d not found in world (might be pending spawn)", entity_id))
        return
    end
    ...
    for comp_name, comp_data in pairs(payload.components or {}) do
        ...
        if is_server then
            ...
            if authority ~= "client" then
                print(string.format("[NET] Server rejected update for %s (authority=%s)", comp_name, authority))
                goto continue_update
            end
        elseif is_own_entity then
            ...
            if authority ~= "server" then
                print(string.format("[NET] Owner rejected update for %s (authority=%s)", comp_name, authority))
                goto continue_update
            end
        end
...
local function handle_despawn(world, msg, owner_client)
    ...
    if entity_id then
        print(string.format("[NET] Despawning entity %d (net_id %d)", entity_id, net_id))
        despawn(entity_id)
...
local function handle_owner_change(world, msg, owner_client)
    ...
    if entity_id then
        ...
        if entity then
            ...
            print(string.format("[NET] Owner changed for %d to %s", net_id, tostring(new_owner)))
        end
    end
end
...
local function handle_client_id(world, msg)
    ...
    print(string.format("[NET] Received client_id: %s", tostring(client_id)))
end
...
function Inbound.system(world)
    ...
    for _, net_id in ipairs(timed_out) do
        print(string.format("[NET] Orphaned pending child %d timed out", net_id))
    end
end
