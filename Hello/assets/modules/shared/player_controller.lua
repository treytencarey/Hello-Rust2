...
--   PlayerController.register_systems()

local NetSync = require("modules/net/init.lua")
local CameraController = require("modules/camera/controller.lua")
...
local state = define_resource("PlayerControllerState", {
...
local function camera_attachment_system(world)
    ...
    local my_client_id = NetSync.get_my_client_id()
    if not my_client_id then
        return
    end
    
    -- Query for entities with NetworkSync and PlayerState
    local entities = world:query({ NetSync.MARKER, "PlayerState", "ScriptOwned" })
    ...
    for _, entity in ipairs(entities) do
        ...
        if sync.owner_client == my_client_id then
            print(string.format("[PLAYER] Found my player entity: %d", entity_id))
            ...
            if not entity:has(NetSync.PREDICTION) then
                entity:patch({
                    [NetSync.PREDICTION] = {
                        predictions = {},
...
local function movement_system(world)
    ...
    if is_key_just_pressed(world, "KeyV") then
        ...
        print("[PLAYER] Rotation mode changed to: " .. state.rotation_mode)
    end
    ...
    if transform then
        ...
        local pred_state = entity:get(NetSync.PREDICTION) or { predictions = {} }
        ...
        entity:patch({ [NetSync.PREDICTION] = pred_state })
    end
end
...
print("[PLAYER] Systems registered (disabled by default, call enable())")
...
--- @return number|nil
function PlayerController.get_my_player()
    return state.my_player_entity
end

--- Set movement speed
--- @param speed number
function PlayerController.set_movement_speed(speed)
