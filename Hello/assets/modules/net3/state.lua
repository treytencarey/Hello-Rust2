...
function State.get()
    return define_resource("NetSyncState", {
        -- Entity tracking
        known_entities = {},      -- net_id -> entity_id
        entity_to_net = {},       -- entity_id -> net_id
        entity_sync_config = {},  -- entity_id -> { sync_components, last_sync_times, last_sync_hashes, spawned, created_locally }
...
function State.get_entity_id(net_id)
    return State.get().known_entities[net_id]
end
...
function State.get_net_id(entity_id)
    return State.get().entity_to_net[entity_id]
end
...
function State.is_known(net_id)
    return State.get().known_entities[net_id] ~= nil
end
...
function State.get_known_clients()
    return State.get().known_clients
end
...
function State.set_owner(net_id, client_id)
    State.get().net_id_owners[net_id] = client_id
end
...
function State.get_owner(net_id)
    return State.get().net_id_owners[net_id]
end

--- Clear owner of a net_id
--- @param net_id number
function State.clear_owner(net_id)
    State.get().net_id_owners[net_id] = nil
end
