-- Conflux3 Server Main Entry Point
...
local config = define_resource("Conflux3ServerConfig", {
    port = 5000,
})
    ...
    for net_id, entity_id in pairs(state.known_entities) do
        if state.net_id_owners[net_id] == client_id then
            ...
            if entity and entity:has("PlayerState") then
                despawn(entity_id)
                print(string.format("[CONFLUX3_SERVER] Despawned player entity %d", entity_id))
            end
        end
    end
end
...
local function on_game_ready()
    print("[CONFLUX3_SERVER] Server ready and listening")
end

local function on_player_joined(client_id, world)
    print(string.format("[CONFLUX3_SERVER] Player joined: %s", client_id))
    spawn_player_for_client(client_id, world)
end

local function on_player_left(client_id, world)
    print(string.format("[CONFLUX3_SERVER] Player left: %s", client_id))
    despawn_player_for_client(client_id, world)
end

-- Host the server
NetGame3.host(config.port, {
    on_game_ready = on_game_ready,
    on_player_joined = on_player_joined,
    on_player_left = on_player_left,
})

print(string.format("[CONFLUX3_SERVER] Hosting on port %d, instance_id: %d", config.port, __INSTANCE_ID__))
